  ...
  useEffect(() => {
    ...
    const loadChatsAndLastActive = async () => {
      try {
        ...
        const lastActiveChatId = await secureStorage.getItem(
          `lastActiveChat_${userId}`
        );
        ...
        const chatToLoad =
          lastActiveChatId && loadedChats.find((c) => c.id === lastActiveChatId)
            ? lastActiveChatId
            : loadedChats[0].id;

        setCurrentChatId(chatToLoad);
        setMessages(
          loadedChats.find((c) => c.id === chatToLoad)?.messages || []
        );
      } catch (error) {
      ...
      if (segment.startsWith("**") && segment.endsWith("**")) {
        ...
        return (
          <View key={index} style={styles.codeBlock}>
            <Text style={styles.codeText}>{segment.slice(3, -3)}</Text>
          </View>
        );
      }
      // Regular text
